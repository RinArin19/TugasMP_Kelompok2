...
public class EstadisticasFragment extends Fragment
{
    ...
    String[] opciones = {"Number of sales", "Revenue in dollars", "Profit in dollars"};
    ...
    {
        Toast.makeText(getActivity(), "Sale", Toast.LENGTH_SHORT);
        ...
        barDataSet = new BarDataSet(cambioVenta, "Amount of Sales");
    ...
    {
        Toast.makeText(getActivity(), "Income", Toast.LENGTH_SHORT);
        cambioVenta.clear();
        barChart.invalidate();
        barChart.clear();
        for (int i = 0; i < 7; i++)
        ...
        barChart.invalidate();
        barDataSet = new BarDataSet(cambioVenta, "Daily Income");
        barDataSet.setColor(getResources().getColor(R.color.bars));
        YAxis rightYAxis = barChart.getAxisRight();
        rightYAxis.setEnabled(false);
        XAxis xAxis = barChart.getXAxis();
    ...
    {
        Toast.makeText(getActivity(), "Profits", Toast.LENGTH_SHORT);
        cambioVenta = new ArrayList<BarEntry>();
        for (int i = 0; i < 7; i++)
        {
            BarEntry b = new BarEntry(i, listaIngresos[i]);
        ...
        barDataSet = new BarDataSet(cambioVenta, "Daily Profits");
        barDataSet.setColor(getResources().getColor(R.color.bars));
        YAxis rightYAxis = barChart.getAxisRight();
        rightYAxis.setEnabled(false);
        XAxis xAxis = barChart.getXAxis();
    ...
    private void refrescarEstadisticas()
    {
        ...
        if (Articulo.cantidadArticulosRegistrados() > 0 && Venta.cantidadVentasRegistradas() > 0)
        {
            if (objMas != null && objMenos != null)
            {
                ...
                if (masV != null)
                {
                    ...
                    cantidadMasVendido.setText(((int) objMas[1]) + " units.");
                ...
                if (menosV != null && !(menosV.getDescripcion().equals(masV.getDescripcion())))
                {
                    ...
                    cantidadMenosVendido.setText(((int) objMenos[1]) + " units.");
        ...
        if (objMasVentas != null)
        {
            ...
            if (mayorCantVentas > 0)
            {
                ...
                ventas_diaMasVentas.setText(mayorCantVentas + " sales.");
            }
            else
            {
        ...
        if (objMenosVentas != null)
        {
            ...
            if (!diaMenosV.equals(diaMasV) && !diaMasVentas.getText().equals("-"))
            {
                ...
                ventas_diaMenosVentas.setText(menorCantVentas + " sales.");
            }
            else
            {
