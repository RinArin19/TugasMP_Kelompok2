...
public class CrearProductoActivity extends AppCompatActivity
{
    ...
    public boolean validarDatos()
    {
        ...
        {
            cantidadView.setError("Check this field.");
            todoBien = false;
        }
        ...
        if (!descripcion_original.equals(articulo.getDescripcion()))
        {
            ...
            if (articulo.obtenerId() == -1)
            {
                ...
                if (descripcion_original.equals(""))
                {
                    ...
                    {
                        Toast.makeText(getApplicationContext(), "Artikel berhasil didaftarkan", Toast.LENGTH_SHORT).show();
                    }
                    else
                    {
                        Toast.makeText(getApplicationContext(), "ERROR saat mendaftarkan artikel", Toast.LENGTH_SHORT).show();
                    }
                    finish();
                }
                else
            ...
            else
            {
                ...
                {
                    mensaje = "Produk dengan deskripsi ini sudah ada, ganti?";
                }
                //Desea sobreescribir producto inactivo??
                else
                {
                    mensaje = "Ada produk tidak aktif dengan deskripsi ini, timpa dan perbarui?";
                }
